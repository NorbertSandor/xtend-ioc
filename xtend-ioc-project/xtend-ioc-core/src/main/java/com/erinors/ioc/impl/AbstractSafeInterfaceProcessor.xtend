...
{
	val AbstractInterfaceProcessor abstractInterfaceProcessor
		...
		extension RegisterGlobalsContext context)
	{
		...
		{
			abstractInterfaceProcessor.doRegisterGlobals(
				annotatedSourceElements as List<? extends InterfaceDeclaration>, context)
		}
	}
		...
		extension TransformationContext context)
	{
		...
		{
			abstractInterfaceProcessor.doTransform(
				annotatedTargetElements as List<? extends MutableInterfaceDeclaration>, context)
		}
	}
		...
		extension CodeGenerationContext context)
	{
		if (check(annotatedSourceElements, null))
		{
			abstractInterfaceProcessor.doGenerateCode(annotatedSourceElements as List<? extends InterfaceDeclaration>,
				context)
		}
	}
	...
	override doValidate(List<? extends Declaration> annotatedTargetElements, extension ValidationContext context)
	{
		...
		{
			abstractInterfaceProcessor.doValidate(annotatedTargetElements as List<? extends InterfaceDeclaration>,
				context)
		}
	}
	...
	def private check(List<? extends Declaration> declarations, ProblemSupport problemSupport)
	{
		...
		invalidDeclarations.forEach [
			...
			{
				problemSupport.addError(it, '''@«annotationClass.simpleName» is supported only for interface declarations. [«errorCode»]''')
			}
		]
